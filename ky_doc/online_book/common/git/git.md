&emsp;&emsp;简单介绍：git是一个**版本控制**工具，所谓的版本控制其实就是为了方便管理你的代码。通俗些讲有点类似游戏里的存档。当你意识到接下来可能要做一些比较麻烦或者难以复原的改动，就可以在开始修改之前先做个存档。之后要是写崩了可以马上跳转回来，非常高效和安全。（如果没有git，我们的做法可能是先复制一份当前代码。之后需要的时候才把他换回去。这样做也行但是并不方便）
